...
class SheetRange(object):
    ...
    def __init__(self, title,
                 ...
                 max_row_p, max_row_idx):
        # None > 0 is False
        if not all(idx > 0 for idx in (min_col_idx, min_row_idx, max_col_idx, max_row_idx)):
            msg = "Values for 'min_col_idx', 'min_row_idx', 'max_col_idx' *and* 'max_row_idx' " \
                  "must be strictly positive"
            raise ValueError(msg)
        # Intervals are inclusive
        if not min_col_idx <= max_col_idx:
            fmt = "Interval [{min_col_idx}, {max_col_idx}] is in reverse order"
            raise ValueError(fmt.format(min_col_idx=min_col_idx, max_col_idx=max_col_idx))
    ...
    def from_string(cls, range_string):
        ...
        min_col_idx = column_index_from_string(mo.group('min_col'))
    ...
    @property
    def coord(self):
        if self.min_col_idx == self.max_col_idx and self.min_row_idx == self.max_row_idx:
            col = self.min_col_p + get_column_letter(self.min_col_idx)
            row = self.min_row_p + str(self.min_row_idx)
            return col + row
        else:
            min_col = self.min_col_p + get_column_letter(self.min_col_idx)
    ...
    def __copy__(self):
        return self.__class__(self.title,
                              self.min_col_p, self.min_col_idx,
    ...
    def shift(self, other):
        ...
        if isinstance(other, tuple):
            ...
            if self.min_col_idx + col_shift <= 0 or self.min_row_idx + row_shift <= 0:
                raise ValueError("Invalid shift value: {0}".format(other))
            self.min_col_idx += col_shift
    ...
    def __ne__(self, other):
        ...
        if isinstance(other, SheetRange):
            ...
            return (ne_sheet_title or
                    ...
                    other.min_col_idx != self.min_col_idx or self.max_col_idx != other.max_col_idx)
        raise TypeError(repr(type(other)))
    ...
    def issubset(self, other):
        ...
        if isinstance(other, SheetRange):
            ...
            return (eq_sheet_title and
                    ...
                    (other.min_col_idx <= self.min_col_idx <= self.max_col_idx <= other.max_col_idx))
        raise TypeError(repr(type(other)))
    ...
    def issuperset(self, other):
        ...
        if isinstance(other, SheetRange):
            ...
            return (eq_sheet_title and
                    ...
                    (self.min_col_idx <= other.min_col_idx <= other.max_col_idx <= self.max_col_idx))
        elif isinstance(other, tuple):
            row_idx, col_idx = other  # cell index in worksheet._cells
            return ((self.min_row_idx <= row_idx <= self.max_row_idx) and
                    (self.min_col_idx <= col_idx <= self.max_col_idx))
        raise TypeError(repr(type(other)))
